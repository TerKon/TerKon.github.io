		...
		<table width="80%" cellpadding="2">
			...
			{foreach from=$ban_list_aktiv item=ban_list_aktiv}
				...
				<tr class="list" style="cursor:pointer;" onClick="NewToggleLayer('layer_{$ban_list_aktiv.bid}');">
					...
					<td>{if $ban_list_aktiv.player_id <> "STEAM_ID_LAN"}{$ban_list_aktiv.player_id}{else}{"_NOTAVAILABLE"|lang}{/if}</td>
				...
				<tr id="layer_{$ban_list_aktiv.bid}" style="display: none">
					<td colspan=10><div style="display: none">
							...
							{if $ban_list_aktiv.player_id <> "STEAM_ID_LAN"}
								<tr><td><b>{"_STEAMID"|lang}:</b></td><td>{$ban_list_aktiv.player_id}</td></tr>
								<tr><td><b>{"_STEAMCOMID"|lang}:</b></td><td><a href="http://steamcommunity.com/profiles/{$ban_list_aktiv.player_comid}" target="_blank">{$ban_list_aktiv.player_comid}</a></td></tr>
							{/if}
								...
								<tr><td><b>{"_INVOKED"|lang}:</b></td><td>{$ban_list_aktiv.ban_created|date_format:"%Y-%m-%d"}</td></tr>
								...
								<tr><td><b>{"_EXPIRES"|lang}:</b></td><td>
									{if $ban_list_aktiv.ban_length==0}<i>{"_NOTAPPLICABLE"|lang}</i>{else}{$ban_list_aktiv.ban_end|date_format:"%Y-%m-%d"}
		...
		<table width="80%" align="center" border="1" cellpadding="2">
			...
			{foreach from=$ban_list_exp item=ban_list_exp}
				...
				<tr class="list"  style="cursor:pointer;" onClick="NewToggleLayer('layer_{$ban_list_exp.bid}');">
					...
					<td>{if $ban_list_exp.player_id <> "STEAM_ID_LAN"}{$ban_list_exp.player_id}{else}{"_NOTAVAILABLE"|lang}{/if}</td>
				...
				<tr id="layer_{$ban_list_exp.bid}" style="display: none">
					<td class="table_list" colspan=10><div style="display: none">
							...
							<table width="90%" class="table_details" cellspacing="0">
								...
								{if $ban_list_exp.player_id <> "STEAM_ID_LAN"}
									<tr><td><b>{"_STEAMID"|lang}:</b></td><td>{$ban_list_exp.player_id}</td></tr>
									<tr><td><b>{"_STEAMCOMID"|lang}:</b></td><td><a href="http://steamcommunity.com/profiles/{$ban_list_exp.player_comid}" target="_blank">{$ban_list_exp.player_comid}</a></td></tr>
								{/if}
								...
								<tr><td><b>{"_INVOKED"|lang}:</b></td><td>{$ban_list_exp.ban_created|date_format:"%Y-%m-%d"}</td></tr>
								...
								<tr><td><b>{"_EXPIRES"|lang}:</b></td><td>
									{if $ban_list_exp.ban_length==0}<i>{"_NOTAPPLICABLE"|lang}</i>{else}{$ban_list_exp.ban_end|date_format:"%Y-%m-%d"}
