		...
		{if $smarty.session.amxadmins_view == "yes"}
			...
			{if $web==true}<center><div class="admin_msg"><img src="images/warning.gif" border="0" /> <a href="{$version_db_web.url}" target="_blank">{"_WEBUPDATE_RECOMMENDED"|lang}</a></div></center>{/if}
			{if $plugin==true}<center><div class="admin_msg"><img src="images/warning.gif" border="0" /> <a href="{$version_db_plugin.url}" target="_blank">{"_PLUGINUPDATE_RECOMMENDED"|lang}</a></div></center>{/if}
			{foreach from=$error item=error}
				<div class="admin_msg" style="font-weight:bold;color:red;">{$error|lang}</div>
			{/foreach}
			<br />
		{else}
			{"_NOACCESS"|lang}
