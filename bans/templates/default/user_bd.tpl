	...
	<div class="post">
		<table frame="box" rules="groups" summary=""> 
			...
			<tbody> 
				...
				<tr>
					<td class="fat">{"_INVOKED"|lang}</td><td>{$ban_detail.ban_created|date_format:"%d.%m.%Y - %T"}</td>
				</tr>
				<tr>
			...
			<tbody> 
				{foreach from=$demos item=demos}
					<form method="post">
						...
						<input type="hidden" name="details_x" value="1" />
						<tr>
							<td>{$demos.upload_time|date_format:"%d.%m.%Y - %T"}</td> 
				...
				<input type="hidden" name="details_x" value="1" />
					<table frame="box" rules="groups" summary=""> 
						<tbody> 
							<!-- Comment List -->
							<tr> 
								<td style="width:130px;">{$comments.date|date_format:"%d.%m.%Y - %T"}</td> 
